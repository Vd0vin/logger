...
namespace Preparator
{
    public class Preparator
    {
                            ...
                            if (flag)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (line1.Length > 0 && line2.Length > 0)
                                    {
                                        ...
                                        var list3 = ParseOrRestoreNumber(line1);
                                        var list4 = ParseOrRestoreNumber(line2);
        ...
        public static void PrepareAfterDecompress(string outputPath, string newOutputPath)
        {
            ...
            {
                char[] charArray = null;
                using (StreamWriter writer = new StreamWriter(newOutputPath, false, new UTF8Encoding(false)))
                {
                    while (reader.Peek() != -1)
                    {
                        charArray = new char[1024];
                            ...
                            if (flag)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        resultsb.Append(ParseOrRestoreNumber(list3, list4));
        ...
        public static string ParseOrRestoreNumber(params string[] list)
        {
            StringBuilder result = new StringBuilder();
            if (list.Length == 1)
            {

                var lengthLimit = list[0].Length > 44 ? 20 : list[0].Length - 24;
                var numberChunk = list[0].Substring(24, lengthLimit);
            ...
            else
            {
                ...
                {
                    if (char.IsDigit(list[0][i])) number1.Append(list[0][i]);
                    else break;
                }

                for (int i = 0; i < list[1].Length; i++)
                {
                    if (char.IsDigit(list[1][i])) number2.Append(list[1][i]);
                    else break;
                }
